...
import de.nlinz.xeonInventory.classes.CGameMode;
        ...
        InventoryPlugin.inst().getServer().getScheduler().runTaskAsynchronously(InventoryPlugin.inst(), new Runnable() {
            public void run() {
                ...
                if (!SQLInject.hasInventory(player.getUniqueId())) {
                    ...
                    CookieApi.sendInfo(player, I18n.translate("messages.loginStats", CGameMode.getGameModeInfo(player), CFlyMode.getFlyModeInfo(player), CVanishMode.getVanishModeInfo(player)));
                    return;
                }
