package de.nlinz.xeonInventory.database;
...
import de.nlinz.xeonInventory.utils.InventoryHistory;
        ...
        try {
            ...
            if (unlock) {
                PreparedStatement sql = conn.prepareStatement("UPDATE inventoryData SET lockState = '" + 0 + "', time = '" + now.getTime() + "', inventoryData = '" + invData + "', armorData = '"
                        ...
                        + health + "', food = '" + hunger + "', gamemodeData = '" + gamemodeData + "', fireticks = '" + fireticks + "', slot = '" + slot + "', fly = '" + fly + "', vanish = '" + vanish
                        + "' " + "WHERE uuid = '" + uuid.toString() + "';");
                sql.executeUpdate();
                sql.close();

            } else {
                PreparedStatement sql = conn.prepareStatement("UPDATE inventoryData SET time = '" + now.getTime() + "', inventoryData = '" + invData + "', armorData = '" + armorData
                        + "', enderData = '" + endData + "', potionData = '" + potionData + "', level = '" + level + "', exp = '" + exp + "', maxHealth = '" + maxHealth + "', health = '" + health
                        + "', food = '" + hunger + "', gamemodeData = '" + gamemodeData + "', fireticks = '" + fireticks + "', slot = '" + slot + "', fly = '" + fly + "', vanish = '" + vanish + "' "
                        + "WHERE uuid = '" + uuid.toString() + "';");
                sql.executeUpdate();
                sql.close();
            }
        ...
        try {
            ...
            PreparedStatement sql1 = conn.prepareStatement(
                    "INSERT INTO inventoryHistory (uuid, timeStamp, inventoryData, armorData, enderData, potionData, level, exp, maxHealth, health, food, gamemodeData, fireticks, slot, fly, vanish) VALUES ('"
                            + uuid + "', '" + now.getTime() + "', '" + invData + "', '" + armorData + "', '" + endData + "', '" + potionData + "', '" + level + "', '" + exp + "', '" + maxHealth
                            + "', '" + health + "', '" + hunger + "', '" + gamemodeData + "', '" + fireticks + "', '" + slot + "', '" + fly + "', '" + vanish + "');");
            sql1.executeUpdate();
            sql1.close();
